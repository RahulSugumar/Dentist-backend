...
def book_appointment(appt: AppointmentCreate):
    try:
        ...
        try:
            ...
            start_dt_naive = datetime.datetime.strptime(f"{appt.appointment_date} {appt.appointment_time}", "%Y-%m-%d %H:%M")
            
            # Make it timezone aware (IST)
            start_dt = start_dt_naive.replace(tzinfo=ist_offset)
            end_dt = start_dt + datetime.timedelta(hours=1)
            
            # Format explicitly for ISO 8601 (will now include +05:30)
